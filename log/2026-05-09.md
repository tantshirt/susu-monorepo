...
All 7 stories merged across 6 waves on 2026-05-09. Sprint-status flipped `epic-8: done`. Submission marketing surface (README hero with badge architecture and HTML-comment sentinels, AdversaryBadge + UpgradeBurnedBadge SVG routes, animated curve hero + interactive variant + cartel toggle, link cluster with `latest-log` mirror + markdown-link-check CI, demo video scaffolding with Git LFS placeholder + bilingual VTT + click-to-play poster, partner outreach machinery) is now on `main`.

**Epic 8 outstanding follow-ups:**
- User to record real 60–90s demo video → drop into `apps/reference/public/demo.mp4` (LFS), paste YouTube ID into README, drop the `youtube.*TODO` ignore pattern.
- User to run partner outreach (Squads/Privy/Helius/Token Extensions). Successful confirmation → `scripts/swap-partner-reference.sh --partner <name> --url <url>`. No confirmation by submission close → `--drop`.
- Pre-existing CI flakes recap: `axe-core WCAG 2.1 AA` dev-server-startup timeout (deferred fix); Playwright visual-regression `/en 404` should be cleared by `0cd43cc` (i18n.ts → next-intl 4.x `requestLocale`); next-PR will confirm.

Now scoping orchestration to Epic 9 (Mainnet Deploy & Immutability Gate). Serial flow: 9.1 → 9.2 (HARD STOP for user authorization before mainnet deploy + set-upgrade-authority --final) → 9.3 → 9.4 (tag v0.1.0-mainnet).

## Story 9.1 audit sign-off gate verification (PR #217)

- Added `scripts/check-audit-signoff.sh` with two modes: enforce (default — fails if audit firm engagement, audit report PDF, findings tracker, or sign-off artifact is missing) and skip (env `SUSU_AUDIT_GATE=skip` OR `audits/SKIP_AUDIT_GATE` sentinel present — exits 0 with a "skipped (pre-audit)" message). Accepts both `audits/SIGNED_OFF` (non-empty file) OR `signed_off: true` + `signed_off_at: <date>` in `audits/audit-summary.json` so the eventual audit firm landing PR can pick either shape.
- Added `audits/SKIP_AUDIT_GATE` sentinel (one-line explanation pointing to Story 9.2's authorization requirement). Story 9.2 will delete this sentinel and require the gate to pass for real.
- Added `.github/workflows/audit-signoff.yml` — paths-filtered CI workflow (runs on PRs touching `programs/susu/**`, `Anchor.toml`, `.github/workflows/release.yml`).
- `package.json` adds `audit:check` script. CONTRIBUTING.md documents the gate.
- Tests 10/10 ATDD red→green; lint-and-build green; merged at SHA `d6a20047f368142cfc6e18f4406c3177dfc00683`.
- **Caveat for 9.2:** the mainnet-deploy preflight MUST delete `audits/SKIP_AUDIT_GATE` and confirm `bash scripts/check-audit-signoff.sh` then passes; if it fails, the audit isn't done and the operator should be surfaced the explicit blocker (missing items: `audits/audit-summary.json`, report PDF, sign-off artifact). 9.2 also wires the gate into `.github/workflows/release.yml` as a hard precondition (skipped in this PR to avoid circular pass via SKIP sentinel).
